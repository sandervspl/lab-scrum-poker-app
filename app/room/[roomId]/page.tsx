...
import { POKER_VALUES, type Participant, type Vote, type Room } from "@/types"
...
import { Users, Copy, Check, Pencil, Eye, EyeOff, Trash2 } from "lucide-react"
...
export default function RoomPage({ params }: { params: { roomId: string } }) {
  ...
  const toggleVotesVisibility = async () => {
    ...
    const { error } = await supabase.from("rooms").update({ votes_revealed: newValue }).eq("id", roomId)
  ...
  const updateRoomName = async () => {
    ...
    const { error } = await supabase.from("rooms").update({ room_name: editedRoomName.trim() }).eq("id", roomId)
  ...
  return (
    <div className="bg-gradient-to-br from-background via-background to-muted/20 p-4 md:p-8 min-h-[calc(100vh-3.5rem)]">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mt-1">
              {isEditingRoomName ? (
                ...
                <p className="text-muted-foreground">{room?.room_name || `Room: ${roomId.slice(0, 8)}`}</p>
              )}
            ...
            <div className="grid grid-cols-5 sm:grid-cols-10 gap-3">
              {POKER_VALUES.map((value) => (
                <button
                  ...
                  className={cn(
                    "aspect-[3/4] rounded-lg border-2 flex items-center justify-center text-2xl font-bold transition-all hover:scale-105 hover:shadow-lg",
          ...
          <CardContent>
            <div className="space-y-3">
              {sortedParticipants.map((participant) => {
                ...
                return (
                  <div key={participant.id} className="flex items-center gap-4 p-3 rounded-lg bg-muted/50">
